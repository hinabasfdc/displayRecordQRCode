...
export default class qrcode {
   ...
   * @param {integer} cellSize The size of a module in pixels.
   * @param {integer} margin The margin to draw around the QR code in pixels.
   * @returns {String} An base64 img data as a string.
   * atelierhi
   */
  getBase64GifImageData(cellSize, margin) {
    cellSize = cellSize || 2;
    margin = typeof margin == "undefined" ? cellSize * 4 : margin;
    ...
    return _getBase64GifImageData(size, size, function(x, y) {
      if (min <= x && x < max && min <= y && y < max) {
        let c = Math.floor((x - min) / cellSize);
        let r = Math.floor((y - min) / cellSize);
        return self.isDark(r, c) ? 0 : 1;
...
/*
* Added Function Returning Base64
* atelierhi
*/
const _getBase64GifImageData = function(width, height, getPixel) {
  let gif = gifImage(width, height);
  for (let y = 0; y < height; y += 1) {
    for (let x = 0; x < width; x += 1) {
      gif.setPixel(x, y, getPixel(x, y));
